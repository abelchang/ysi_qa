    ...
    <div
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <form class="form-horizontal" method="post" action="">
                {{ csrf_field() }}
                @foreach ($qa->questions as $item)
                <label class="col-md-2">{{$item['title']}}:</label>
                <div class="col-md-6">
                    @foreach ($item['aoptions'] as $option)
                    <input type="radio" name="{{$item['no'].$option['title']}}" value="{{$option['no']}}">
                    {{$option['title']}}<br>
                    @endforeach
                </div>
                @endforeach
            ...
            <select class="form-control" name="resoureceName">
                ...
                @foreach ($qa->questions as $item)
                <option value="{{ $item['no'] }}" {{ ( $item['no']==0) ? 'selected' : '' }}> {{
                    $item['title'] }}
                </option>
                @endforeach
            </select>
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4 text-lg leading-7 font-semibold"><a href="https://laravel.com/docs"
                                    class="underline text-gray-900 dark:text-white">{{$project->name}}</a></div>
                        </div>
